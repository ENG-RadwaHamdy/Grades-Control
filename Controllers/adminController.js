...
const addAdmin = (req, res) => {
  ...
  adminModel.insertAdmin(admin, (err, adminId) => {
    ...
    return res.status(200).json({ message: 'Admin added successfully', adminId });
  });
};
...
const addCourse = (req, res) => {
  ...
  courseModel.insertCourse(course, (err, courseId) => {
    ...
    return res.status(200).json({ message: 'Course added successfully', courseId });
  });
};
  ...
  upload.single('file')(req, res, (err) => {
    ...
    data.forEach((row) => {
      ...
      studentModel.getStudentById(National_ID, (err, existingStudent) => {
        ...
        if (existingStudent) {
          console.log(`Student with National ID ${National_ID} already exists. Skipping.`);
          return;
        }
    ...
    return res.status(200).json({ message: 'Student data extracted and saved successfully' });
  });
};
...
const saveGrades = async (req, res, next) => {
  try {
    ...
    for (const gradeData of Grades) {
      const { National_ID, Grade, Oral_degree, Year_Work, Full_Grade, Practical_Exams_Grade, Written_Exams_Grade } = gradeData;
      ...
      db.run(
        ...
        [Course_Name, National_ID, Grade, Oral_degree, Year_Work, Full_Grade, Practical_Exams_Grade, Written_Exams_Grade],
        function (err) {
          if (err) {
  ...
  try {
    db.all(
      ...
      (err, rows) => {
        ...
        rows.forEach((row) => {
          const { National_ID, Student_Name, Course_Name, Grade, Year_Work, Full_Grade, Practical_Exams_Grade, Written_Exams_Grade, Oral_degree } = row;
